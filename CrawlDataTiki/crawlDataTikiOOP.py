...
class TikiCrawler:
    ...
    def crawl_tiki_data(self, num_pages):
        for i in range(1, num_pages + 1):
            ...
            if response.status_code == 200:
                ...
                self.save_to_json('products_tiki.json')

                time.sleep(random.randrange(1, 3))

    def send_request(self):
        return requests.get('https://tiki.vn/api/personalish/v1/blocks/listings',
...
if __name__ == "__main__":
    class MyTikiCrawler(TikiCrawler):
        def __init__(self):
            super().__init__()

        def crawl_tiki_data(self, num_pages):
            super().crawl_tiki_data(num_pages)

    crawler = MyTikiCrawler()
    num_pages_to_crawl = 27  # Số trang bạn muốn crawl
    crawler.crawl_tiki_data(num_pages_to_crawl)
